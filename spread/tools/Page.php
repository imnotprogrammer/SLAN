	...
	*/
	class Pages {
		...
		function fpage(){
			...
			$html[0] = "<span class='page-tip'>共<b> {$this->total} </b>{$this->config["head"]}，";
			$html[1] = "每页显示： <b>".$this->disnum()."</b> 条</span>";
        ...
        function appPage(){
            ...
            if( $this->pageNum > 1 ){
                if( $this->page >1 ){
                    $html[] = sprintf( '<a href="%spage=1" data-ajax="false" class="ui-btn ui-corner-all">首页</a>
                                        <a href="%spage=%d" data-ajax="false" class="ui-btn ui-corner-all">上页</a>',
                                        $this->uri,$this->uri,$this->page-1
                               
                                );
                }
                if( $this->page < $this->pageNum ){
                    $html[] = sprintf('<a href="%spage=%s" data-ajax="false" class="ui-btn ui-corner-all">下页</a>
                                      <a href="%spage=%d" data-ajax="false" class="ui-btn ui-corner-all">尾页</a>',
                                      $this->uri,$this->page+1,$this->uri,$this->pageNum
                               );
                }
            }
            $html[] = sprintf('</div><p class="app-page-tip" style="padding-left:20px;">共%d个记录，目前是第%d页</p>',$this->total,$this->page);
            $str = implode('',$html);
			return $str;
		}
        function spage(){
